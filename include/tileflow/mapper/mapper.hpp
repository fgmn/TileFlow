...
struct Reward {
    double value;
    double reward;
    Reward(double reward = 0.0, double value = 0.0) : value(value), reward(reward) {}
    bool operator>(const Reward &other) const { return reward > other.reward; }
    bool operator>(double reward_) const { return reward > reward_; }
    Reward operator-(const Reward &other) const { return {reward - other.reward, value - other.value}; }
    Reward operator+(const Reward &other) const { return {reward + other.reward, value + other.value}; }
    Reward operator/(unsigned n) const { return {reward / n, value / n}; }
    void operator+=(const Reward &other) { reward += other.reward; value += other.value; }
};
...
struct State {
    ...
    inline bool is_error_out() const { return variable_index == ERROR_OUT; }
};
...
public:
    ...
    const SymbolTable* get_best_symbol_table() const { return best_symbol_table_; }
...
public:
    MCTS(unsigned timeout = 120, bool random = false) : Algorithm(timeout), random_(random) {}
    void search() override;
    void set_env(Env* env) override { env_ = env; }
};

struct RLAlgorithm : public Algorithm {
public:
    RLAlgorithm(unsigned timeout = 120) : Algorithm(timeout) {}
    
    void search() override {
        start_timer();
        while (get_elapsed_time() < timeout_) {
            // Implement reinforcement learning search logic here
            // For example, you can use Q-learning or any other RL algorithm
            // This is a placeholder for the RL search logic
            std::cout << "RL search iteration..." << std::endl;
        }
    }
...
public:
    Mapper(const std::vector<Constraint>& constraints_,
        ...
        timeout_(timeout), topk_(topk) {
            ...
            else if (alg == "rl" || alg == "RL")
                alg_ = std::make_unique<RLAlgorithm>(timeout);
